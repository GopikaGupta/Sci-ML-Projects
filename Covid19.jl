#INTRODUCTION
#In this tutorial, we will construct a Universal Differential Equation (UDE) based on the SIR model of epidemiology; with the 
#aim of capturing quarantine effects in the region under consideration. We will show that the resulting UDE is not only good 
#at estimation, but can be used as a powerful diagnostic tool. Using this tool, we can understand the real-time, data driven 
#quarantine variation for any region. In this example, we choose Italy.

#PROCEDURE
#First, we need to decide how to construct the UDE model. We will start with the SIR model which has no information regarding 
#quarantine effects. We will integrate a neural network in the SIR model to capture these effects. Intuitively, we know that 
#since quarantine (or isolated) people don't come into contact with the infected people, we need to subtract a term from the 
#dI/dt equation in the SIR model. Thus, we will inject a neural network as a subtraction term in the infected compartment.

#Second, we will train the resulting UDE on the underlying infected and recovered case count data available publicly from the 
#John Hopkins repository.
#Thirdly, after the model is trained, we will plot the neural network recovered quarantine strength with time and see how 
#it relates to the on ground situation in Italy.

#INSTALLING LIBRARIES
using MAT
using Plots, Measures, Lux, ComponentArrays
using DifferentialEquations
using DiffEqFlux, Optimization, OptimizationFlux,OptimizationOptimJL
using Random

#DATA IMPORT AND CLEANUP
vars = matread("bootcamp/HandsOnProject/Rise_Italy_Track.mat")
Random.seed!(50)
